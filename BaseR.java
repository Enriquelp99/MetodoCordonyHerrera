...
/* -------------------------------------------------------------------------
  ...
  ------------------------------------------------------------------------- */
    ...
    public void decodifica(Vector<TipoRegla> Conjunto_Reglas) {
        int i, j;

        for (i = 0; i < n_reglas; i++) {
            for (j = 0; j < tabla.n_variables; j++) {
                BaseReglas[i][j].x0 = base_datos.BaseDatos[j][Conjunto_Reglas.get(i).
                                      Regla.get(j)].x0;
                BaseReglas[i][j].x1 = base_datos.BaseDatos[j][Conjunto_Reglas.get(i).
                                      Regla.get(j)].x1;
                BaseReglas[i][j].x2 = base_datos.BaseDatos[j][Conjunto_Reglas.get(i).
                                      Regla.get(j)].x2;
                BaseReglas[i][j].x3 = base_datos.BaseDatos[j][Conjunto_Reglas.get(i).
                                      Regla.get(j)].x3;
